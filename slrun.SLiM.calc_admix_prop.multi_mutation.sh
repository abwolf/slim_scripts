...
#SBATCH --qos=1day
#SBATCH --output=slrun.SLiM.calc_admix_prop.multi_mutations.sh.%A_%a.o
source ~/.bashrc
...
for recomb in 0.5e-8 1.0e-8 1.3e-8; do
	...
	for selec in -0.0001 -0.0005 -0.001 -0.005 -0.01 -0.05 -0.08; do
		zcat Tenn_deleterious_output.admix_*.chr_ALL.m3_10.selec_"$selec".recomb_"$recomb".ALL.slim.Neand.bed.merged.gz \
